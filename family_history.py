...
def main():
    ...
    count_marriages_dict = {}
        # Total Marriages per person
    ...
    # Include an additional dictionary to summarize the number of marriages per person
    print_marriages(marriages_dict, people_dict, count_marriages_dict)

    # Print a blank line.
    print()    
    
    # Print the number of marriages per person
    print_count_marriages(count_marriages_dict)

# This function summarizes the number of marriages per person
# It's call during the printing of marriages
# Dictionaries can't have a duplicate key (Person)
def count_marriages(count_marriages_dict,person):
    
    if person in count_marriages_dict:
       value = count_marriages_dict[person]
       value += 1
       count_marriages_dict[person]=value
    else:
        count_marriages_dict[person] = 1
    pass

def print_count_marriages(count_marriages_dict):
    for item in count_marriages_dict.items():
        key = item[0]
        value = item[1]
        print(f'{key} {value}')
    pass
...
def count_genders(people_dict):
    ...
    for item in people_dict.items():
        ...
        if gender.lower() == 'f':
            ...
            male += 1
    print(f'Total Female: {female}  Total Male: {male}')
    pass


def print_marriages(marriages_dict, people_dict, count_marriages_dict):
    ...
    Parameters
        ...
        people_dict: a dictionary that contains data about people
            Each item in the dictionary is in this format:
              person_key: [name, gender, birth_year, death_year]
         count_marriage_dict contains the number of marriages per person
         person_key_name: [count]  
    Return: nothing
    """
    # marriage_key: [husband_key, wife_key, wedding_year]
    for item in marriages_dict.items():
        ...
        if husband_key in people_dict:
            value2 = people_dict[husband_key]
            husband_name = value2[0]
            husband_birth = value2[2]
            husband_age = wedding_year - husband_birth
            count_marriages(count_marriages_dict,husband_name)
        if wife_key in people_dict:
            value2 = people_dict[wife_key]
            wife_name = value2[0]
            wife_birth = value2[2]
            wife_age = wedding_year - wife_birth
            count_marriages(count_marriages_dict,wife_name)
        print(f'{key} {husband_name} {husband_age} - {wedding_year} - {wife_name} {wife_age}')
